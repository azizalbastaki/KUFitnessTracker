...
public class AdminPage extends JFrame implements ActionListener {
    ...
    //         @Override
    //         public boolean isCellEditable(int row, int column) {
    //             return column == 6; // Editable only for the "Status" column
    //         }
    //     };
    ...
    //             // Get the name of the activity from the table (assuming it's in the first column)
    //             String activityName = (String) goalTable.getValueAt(selectedRow, 0);
    ...
    //     goalTable.getColumnModel().getColumn(5).setCellEditor(new DefaultCellEditor(statusComboBox));
    ...
    //     searchButton.addActionListener(e -> searchGoal(searchField.getText(), activityModel));
    //     searchPanel.add(searchField);
    //     searchPanel.add(searchButton);
    ...
    // // Search Method for Goals
    // private void searchGoal(String searchQuery, DefaultTableModel goalsModel) {
    //     // First, clear the existing rows in the table
    //     goalsModel.setRowCount(0);

    //     // Iterate over the list of goals (assuming 'selectedUser' is the currently selected user)
    //     for (Goal goal : selectedUser.getGoals()) {
    //         // Check if the goal description contains the search query (case-insensitive)
    //         if (goal.getGoalDescription().toLowerCase().contains(searchQuery.toLowerCase())) {
    //             // If there's a match, add the goal to the table model
    //             Object[] row = {
    //                 goal.getGoalDescription(),
    //                 goal.getTargetValue(),
    //                 goal.getProgress(),
    ...
    //                 goal.getStatus()
    //             };
    //             goalsModel.addRow(row);
    //         }
    //     }

    //     // If no goals match, you can show a message or handle accordingly
    //     if (goalsModel.getRowCount() == 0) {
    //         JOptionPane.showMessageDialog(null, "No goals found matching the search query.");
    //     }
    // }

    // // Simulated methods for goal actions
    // private void loadGoalsIntoTable(DefaultTableModel model) {
    //     // Check if the model or selectedUser is null
    //     if (model == null) {
    //         throw new IllegalArgumentException("Table model cannot be null.");
    //     }
    //     if (selectedUser == null || selectedUser.getGoals() == null) {
    //         System.out.println("No goals to load as the selected user or their goals are null.");
    //         return;
    //     }
    
    //     // Populate the table model with goals
    //     selectedUser.getGoals().forEach(goal -> {
    //         int progressValue = (goal.getTargetValue() > 0)
    //                 ? (int) ((double) goal.getProgress() / goal.getTargetValue() * 100)
    //                 : 0;
    
    //         // Add the goal data to the table
    //         model.addRow(new Object[]{
    //             goal.getGoalDescription(),
    //             goal.getTargetValue(),
    //             goal.getProgress(),
